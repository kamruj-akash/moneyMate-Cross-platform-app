...
// Build the full backup payload + a stable filename. Shared between the
// share and save-to-device flows below.
const buildBackup = ({ transactions, categories, recurring, settings }) => {
  const payload = {
    schema_version: 1,
  ...
  const fileName = `moneymate-backup-${ts}.json`;
  return { payload, fileName, content: JSON.stringify(payload, null, 2) };
};

// Existing share flow — surfaces the system share sheet so the user can
// hand the file to Drive / WhatsApp / Save-to-Files / etc.
export const exportBackupJSON = async (data) => {
  const { fileName, content } = buildBackup(data);
...
// Save the backup to a user-chosen folder on the device. On Android we use
// the Storage Access Framework — the user picks a directory once, we write
// directly to it, and the file shows up in the device's Files app like any
// other download. On iOS there's no SAF equivalent, so we fall back to the
// share sheet (which exposes "Save to Files" as a destination — same end
// result for the user, just one extra tap).
//
// Returns { ok, fileName, uri?, reason? }. `reason` is 'cancelled' when the
// user dismisses the directory picker, 'unavailable' when sharing isn't
// possible, or 'error' for anything thrown along the way.
export const saveBackupToDevice = async (data) => {
  const { fileName, content } = buildBackup(data);

  if (Platform.OS === 'android') {
    try {
      const SAF = FileSystem.StorageAccessFramework;
      if (!SAF) return { ok: false, reason: 'unavailable', fileName };
      const perm = await SAF.requestDirectoryPermissionsAsync();
      if (!perm?.granted) return { ok: false, reason: 'cancelled', fileName };
      const fileUri = await SAF.createFileAsync(perm.directoryUri, fileName, 'application/json');
      await FileSystem.writeAsStringAsync(fileUri, content, {
        encoding: FileSystem.EncodingType.UTF8,
      });
      return { ok: true, uri: fileUri, fileName };
    } catch (e) {
      return { ok: false, reason: 'error', error: e?.message, fileName };
    }
  }

  // iOS / web — use the share sheet. iOS users will pick "Save to Files".
  try {
    const fileUri = `${FileSystem.cacheDirectory}${fileName}`;
    await FileSystem.writeAsStringAsync(fileUri, content, {
      encoding: FileSystem.EncodingType.UTF8,
