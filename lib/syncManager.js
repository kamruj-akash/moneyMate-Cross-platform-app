...
// not a date-only "YYYY-MM-DD"). Postgres `date` columns return the latter and
// would otherwise stomp the time when merged back over a richer local row.
...
const mergeByUpdatedAt = (local = [], cloud = []) => {
  ...
  for (const item of cloud) {
    ...
    if (!existing) {
      ...
      // Preserve the richer time-of-day from whichever side has one. This
      // covers Supabase columns typed as `date` that strip the time on
      // round-trip — without this, a refresh resets the row to 12:00 AM.
      if (
        winner === item &&
        existing.date &&
        hasTimeComponent(existing.date) &&
        !hasTimeComponent(item.date)
      ) {
        map.set(item.id, { ...item, date: existing.date });
      } else {
